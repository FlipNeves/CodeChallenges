...
in some languages this "list" is in fact a string (see the Sample Tests).*/
        public static int? chooseBestSum(int t, int k, List<int> ls) => FindBestRouteDistance(t, k, ls.Where(item => item <= t).ToList(), 0);
        private static int? FindBestRouteDistance(int targetLimit, int townsLeft, List<int> distances, int currentIndex)
        {
            if (townsLeft == 0) return 0;
            if (currentIndex >= distances.Count) return null;

            int? distanceWithCurrent = null;
            if (distances[currentIndex] <= targetLimit)
            {
                int? remaining = FindBestRouteDistance(targetLimit - distances[currentIndex], townsLeft - 1, distances, currentIndex + 1);

                if (remaining != null)
                {
                    distanceWithCurrent = distances[currentIndex] + remaining;
                    if (distanceWithCurrent == targetLimit) return distanceWithCurrent;
                }
            }

            int? distanceWithouCurrent = FindBestRouteDistance(targetLimit, townsLeft, distances, currentIndex + 1);

            return distanceWithCurrent.HasValue && distanceWithouCurrent.HasValue
                ? Math.Max(distanceWithCurrent.Value, distanceWithouCurrent.Value)
                : distanceWithCurrent ?? distanceWithouCurrent;
        }
...
Your algorithm should be able to handle large intervals. All tested intervals are subsets of the range [-1000000000, 1000000000].*/
        public static int SumIntervals((int, int)[] intervals)
            => intervals
            .OrderBy(x => x.Item1)
            .Aggregate(new List<(int, int)>(), (acc, interval) =>
            {
                if (acc.Count == 0 || interval.Item1 > acc[^1].Item2)
                    acc.Add(interval);
                else
                {
                    var last = acc[^1];
                    acc[^1] = (last.Item1, Math.Max(last.Item2, interval.Item2));
                }
                return acc;
            })
